  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        // Only keep items available for online order
        const onlineItems = data.filter(item => item.isOnlineAvailable !== false);
      ...
      <div style={{ 
        ...
        <div className="container">
          ...
          <motion.p 
            ...
            style={{ color: '#222', fontSize: '1.2rem', maxWidth: '600px', margin: '0 auto', fontFamily: 'var(--font-serif)', fontStyle: 'italic' }}
          >
            Hot and fresh, straight to your doorstep.
          </motion.p>
        </div>
      </div>
      ...
      <div className="container" style={{ marginTop: '-2rem', position: 'relative', zIndex: 10 }}>
          ...
          {isLoading ? (
            ...
            <AnimatePresence>
              {filteredMenu.map((item, idx) => (
                ...
                >
                  {/* Pass isOnline=true so it goes to onlineCart */}
                  <MenuCard item={item} isOnline={true} />
                </motion.div>
              ))}
            </AnimatePresence>
          )}
        ...
        {!isLoading && filteredMenu.length === 0 && (
          <div className="text-center" style={{ padding: '4rem 0', color: 'var(--text-secondary)' }}>
            ...
            <p>We couldn't find any dishes available for online delivery in this category.</p>
          </div>
        )}
      </div>
    </div>
